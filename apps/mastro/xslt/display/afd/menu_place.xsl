...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
 ...
 <!-- - - - - - - - - - - - - - - - -->
 <xsl:key name="group_by_place" match="tei:*[contains(@ana, 'func_place')]" use="@n" />
 <!-- - - - - - - - - - - - - - - - -->
 ...
 <xsl:template match="/">
  ...
  <ul class="collapsible">
   ...
   <xsl:choose>
    <xsl:when test="count(//tei:*[contains(@ana, 'func_place')])>0">
     <xsl:for-each
      select="//tei:*[contains(@ana, 'func_place')][count(. | key('group_by_place', @n)[1]) = 1]">
      <xsl:sort select="@n" />
      <li>
       ...
       <ul class="collapsible_body">
        <xsl:variable name="item">
         <xsl:for-each select="key('group_by_place', @n)">
