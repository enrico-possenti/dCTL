    ...
    <ul class="collapsible">
     <xsl:choose>
      <xsl:when test="$counts &lt; 1">
       <li> Nessun risultato per i criteri definiti... </li>
      </xsl:when>
      <xsl:otherwise>
       <xsl:for-each
            ...
            <ul class="collapsible_body">
             <xsl:for-each
              ...
              <li class="widget_box">
               ...
               <xsl:variable name="terms" select="$high" />
               ...
               <xsl:variable name="link">$().mastro('display', '<xsl:value-of select="$doc"
                  ...
                  />','<xsl:value-of select="$high" />','<xsl:value-of select="$label"
                  />','<xsl:value-of select="$terms" />');</xsl:variable>
               ...
               <div class="widget_text">
                <div class="widget_title">
                 <a href="javascript:void(0)" onclick="{$link}" title="{$tooltip_goto}">
                  <xsl:choose>
                   <xsl:when test="(count(str:split($terms, ' ')) = 1) or ($mode='db')">
